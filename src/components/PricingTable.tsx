...
export const PricingTable: React.FC<PricingTableProps> = ({ onOpenCheckout, currency, onToggleCurrency }) => {
  ...
  return (
    <section id="pricing" className="py-24 px-6 relative scroll-mt-20 overflow-hidden">
      {/* Glow Backdrop */}
      ...
      <div className="max-w-3xl mx-auto bg-slate-800/90 rounded-3xl border border-blue-500/50 shadow-2xl overflow-hidden backdrop-blur-md">
        {/* Top Urgency Header */}
        <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-600 text-center py-3.5 font-black tracking-widest uppercase text-white text-sm sm:text-base flex items-center justify-center gap-2">
          <Flame className="w-5 h-5 fill-current text-yellow-300 animate-bounce" />
          <span>🔥 Limited Time Launch Offer</span>
        </div>

        <div className="p-6 sm:p-10 md:p-12">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-2">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-white text-center sm:text-left">Today You Get</h2>
          ...
          <p className="text-slate-400 text-center sm:text-left mb-8 text-base">
            Start creating videos that actually look premium.
          </p>

          {/* List of included bundle items */}
          <div className="space-y-3.5 mb-10 font-medium">
            {BUNDLE_ITEMS.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between border-b border-slate-700/70 pb-3 text-sm sm:text-base">
                <span className="flex items-center gap-2.5 text-slate-200">
                  <Check className="w-4 h-4 text-blue-400 shrink-0" />
                  <span>{item.title}</span>
                </span>
                {item.highlight ? (
                  <span className="text-blue-400 font-bold bg-blue-500/10 px-2.5 py-0.5 rounded text-xs border border-blue-500/20">
                    Included Forever
                  </span>
                ) : (
                  <span className="text-slate-400 line-through font-mono text-xs sm:text-sm">
          ...
          <div className="flex justify-between items-end mb-8 bg-slate-900/80 p-6 rounded-2xl border border-slate-700">
            <div>
              ...
              <p className="text-lg font-bold text-slate-200">Today Only Special Price</p>
            </div>
            <div className="text-right">
              <span className="text-xs font-bold text-emerald-400 uppercase tracking-wider block mb-0.5">97% OFF</span>
